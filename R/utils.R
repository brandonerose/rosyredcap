...
write_xl<-function(DF,RC,path){
  wb <- openxlsx::createWorkbook()
  openxlsx::addWorksheet(wb, "sheet")
  if(nrow(DF)>0){
    DF$redcap_link<-paste0("https://redcap.miami.edu/redcap_v12.0.23/DataEntry/record_home.php?pid=",RC$PID,"&id=",DF[[RC$id_col]],"&arm=1")
    class(DF$redcap_link) <- "hyperlink"
    openxlsx::writeData(wb, sheet = 1, x = DF$redcap_link,startRow = 2,startCol = which(colnames(DF)==RC$id_col))
    DF$redcap_link<-NULL
  }
